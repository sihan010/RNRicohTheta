...
import com.rntheta360.RNTheta360Sdk.model.ImageSize;
...
public class HttpConnector {
    ...
    public InputStream getLivePreview() throws IOException, JSONException {
        ...
        Log.v("livePreview","Returning stream");
        return is;
    }
    ...
    public String getSessionId() {
        ...
        try {
            ...
            // parse JSON data
            JSONObject output = new JSONObject(responseData);
            String state = output.getString("state");
            Log.v("state",state);
            if(state.equals("done")){
                JSONObject results = new JSONObject(output.getString("results"));
                String sessionId = results.getString("sessionId");
                Log.v("sessionId",sessionId);
                boolean isApiSet = setApiVersion(sessionId); // Started session now set api version to 2, only for Theta s and below
                return sessionId;
            }
    ...
    public boolean setApiVersion(String sessionId) {
        HttpURLConnection postConnection = createHttpConnection("POST", "/osc/commands/execute");
        JSONObject input = new JSONObject();
        String responseData;
        String errorMessage = null;
    ...
    public InputStream getLivePreviewMod(String sessionId) throws IOException, JSONException {
        ...
        try {
            ...
            os.flush();
            os.close();
            Log.v("live ------->", String.valueOf(postConnection.getResponseCode()));
            is = postConnection.getInputStream();
        } catch (IOException e) {
            e.printStackTrace();
            String errorMessage = null;
